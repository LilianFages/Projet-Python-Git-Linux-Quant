...
from app.common.macro_news_scoring import (
...
# ---------------------------------------------------------------------
# Macro News Impact Board — Macro Dashboard V2.6
# ---------------------------------------------------------------------
def prepare_event_impact_board(
    ...
    Le scoring est centralisé dans app.common.macro_news_scoring.
    """
    combined = []
    ...
    for event in combined:
        ...
        confirmation_score = scored_event.get("market_score", 0)
        confirmation_details = scored_event.get("market_evidence", [])
        final_priority = scored_event.get("final_priority", "Low")
        final_score = scored_event.get("final_score", 0)
        alert_candidate = scored_event.get("alert_candidate", False)
...
def summarize_event_factor_pressure(event_impact_df: pd.DataFrame) -> list[str]:
    ...
    if not factor_counts.empty:
        ...
        lines.append(
            f"Top market-confirmed factor: {top_factor} with final score {int(top_score)}."
        )
    ...
    strong_confirmed = event_impact_df[
        event_impact_df["Market Confirmation"] == "Strong"
    ]

    if not strong_confirmed.empty:
        lines.append(
            f"{len(strong_confirmed)} event(s) are strongly confirmed by market moves."
        )
...
def render_event_impact_board(
    ...
    with right:
        ...
        if alert_df.empty:
            st.info("No alert candidate detected.")
        else:
